...
For example, the reference sentence 'So, normally, most people begin by orienting themselves to the task' has the following translations:

- `word_level_model`: '<unk>, most people start with the <unk> to the **task**.'
- `bpe_level_model_2000`: 'Normally, most people start with the orienting orientate towards the **combitts**.'
- `bpe_level_model_4000`: 'But, most people start with the orienting to orienting towards the **compit**.'

These examples illustrate the differences between the models' translations well; while the word-level model correctly translates 'task', it has <unk> tokens where the BPE models had rather successful translations ('Normally', 'But', 'orienting'). However, the BPE-level models did not successfully produce the final word 'task', both generating a similar combination of subword units which is not an actual English word. 

Furthermore, despite the numerous gaps in the word-level translations due to the <unk> tokens, the outputs seem overall slightly more grammatically correct and fluent than the BPE model outputs. For example, let us take the reference sentence 'Potable water, arable land, rainforests, oil, gas: they're going away, and they're going away soon, and unless we innovate our way out of this mess, we're going away too.':

- `word_level_model`: '<unk> <unk>, land <unk>, <unk> <unk>, oil, natural gas. They're <unk>, and they're <unk> quickly, and if we don't <unk> our <unk> to <unk> out of this <unk>, <unk> we'll also.'
...
In both BPE-based outputs, the same phrases are repeated multiple times superfluously ('out of this guys out of this guair' in the 2000-vocab model and 'and they are disappeared' in the 4000-vocab model, greatly reducing the fluency. On the other hand, the structure of the word-level output seems much more natural and matches the reference better.
